...
function getLinearEquationRoot(a, b) {
  if (a === 0) {
    throw new Error(
      "The coefficient 'a' cannot be zero for a linear equation."
    );
  }
  const root = -b / a;
  return root;
...
function getRandomInteger(min, max) {
  if (!Number.isInteger(min) || !Number.isInteger(max) || min > max) {
    throw new Error(
      'Invalid input. Both min and max must be integers, ' +
        'and min must be less than or equal to max.'
    );
  }

  const range = max - min + 1;
  const randomInteger = min + Math.floor(Math.random() * range);
